...
| **C4**       | Seasonal playbook / 시즌별 플레이북 생성                     | `monthly_demand`, scenario gaps            | `seasonal_playbook.json`                                  | Create action plans for 4 events (Galaxy launch, appliance peak, Q4 promo, semi-conductor surge). / 4대 이벤트(갤럭시 런칭, 가전 피크, Q4 프로모, 반도체 서지)에 대한 실행 계획을 수립하십시오.                                    |
