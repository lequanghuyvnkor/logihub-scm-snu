...
Korean Freight O/D 2022/2023 ghi nhận **toàn bộ vận chuyển hàng hoá liên vùng tại Hàn Quốc** của tất cả ngành — không phân biệt doanh nghiệp, không phân biệt sản phẩm. Nó phản ánh chính xác cấu trúc địa lý của nhu cầu logistics quốc gia — Seoul/Gyeonggi là vùng trọng yếu, Busan là cụm phía Nam, Pyeongtaek là khu công nghệ cao.

Proxy engine khai thác cấu trúc địa lý này. Nó **không mô phỏng số liệu của bất kỳ doanh nghiệp cụ thể nào** — nó demonstrate rằng khi một doanh nghiệp feed data nội bộ thật vào engine, engine sẽ ra được output có cấu trúc và chất lượng phân tích tương tự với những gì hội đồng đang xem.
...
**Lý do 1 — Bí mật kinh doanh.** Shipment data nội bộ của bất kỳ doanh nghiệp nào đều là tài sản chiến lược. Doanh nghiệp không bao giờ public, không có dataset học thuật nào chứa, và đội sinh viên trong khuôn khổ midterm không thể ký NDA với doanh nghiệp lớn để xin data.

**Lý do 2 — Quy mô data thực tế quá lớn.** Một doanh nghiệp logistics quy mô lớn xử lý hàng triệu shipments/năm × ~30 thuộc tính/shipment = hàng trăm GB data raw. Ngay cả khi có data, infrastructure cần để xử lý vượt khả năng máy tính cá nhân của sinh viên. Proxy data nhỏ hơn 100-1000 lần nhưng đủ để demonstrate kiến trúc.
...
**Hai, sinh outcome 16 mục có cấu trúc và độ sâu của một bản phân tích senior SCM manager.** Số là demonstration-grade (không phải data doanh nghiệp thật) nhưng cấu trúc lập luận, các bảng, các diễn giải, các khuyến nghị đều giống output production.
...
**Một, không nói được số chính xác cho một doanh nghiệp cụ thể.** Proxy engine không trả lời được câu hỏi về tiết kiệm chi phí của một công ty cụ thể. Outcome mẫu (file `outcome_sample_full.md`) là **giả định cho minh hoạ**, không phải kết quả engine chạy trên data doanh nghiệp thật.

**Hai, không validate được seasonal pattern của doanh nghiệp cụ thể.** Bảng chỉ số mùa trong Phần 4 là heuristic ngành — từng doanh nghiệp có thể có seasonal pattern khác (ví dụ peak window dịch chuyển 1-2 tháng), engine proxy không phát hiện được.
...
**Phần 4 — Nhu cầu mùa vụ.** Status: 🟠 Heuristic. Task: bảng chỉ số mùa 12 tháng × 6 dòng sản phẩm trong `engine/config.py`. Mỗi cột trung bình ≈ 1.0 (sai số ±2%). Hàm `expand_to_monthly()` áp dụng đúng. **Disclaimer:** số mùa vụ là heuristic ngành, không phải seasonal pattern của bất kỳ doanh nghiệp cụ thể nào. Owner: Nhóm A. Deadline: 17/5.
...
**Tier 2 — Engine đang chạy trên proxy data, không phải data nội bộ doanh nghiệp.** "Vì shipment data nội bộ của doanh nghiệp là bí mật kinh doanh, đội dùng Korean Freight O/D công khai làm proxy. Cấu trúc phân tích và logic tối ưu hoá là production-grade; số tuyệt đối là demonstration."

**Tier 3 — Engine sẽ ra số chính xác khi feed data thật.** "Khi một doanh nghiệp dùng thật, họ chỉ cần feed shipment data nội bộ vào — kiến trúc engine không đổi, code không đổi, chỉ thay file CSV input. Số sẽ chính xác cho doanh nghiệp đó."
...
**Q: Vậy các con số tiết kiệm chi phí trong outcome có thật không?**

A: Không. Đó là giả định cho minh hoạ. Outcome mẫu được xây dựa trên aggregate Korean Freight O/D (không phải data doanh nghiệp cụ thể nào) để demonstrate engine sẽ làm được gì khi feed data thật. Số tuyệt đối là demonstration-grade — doanh nghiệp dùng thật sẽ ra số chính xác cho mạng lưới của họ.
...
*Tài liệu này là một trong những tài liệu canonical của LogiHub. Cùng với `LogiHub_Engine_v2_Redesign.md` (kiến trúc), `LogiHub_Midterm_Plan_3People_18days.md` (kế hoạch), `LogiHub_Onboarding_Tong_Quan.md` (giới thiệu), và `outcome_sample_full.md` (outcome mẫu industry-agnostic), nó tạo thành bộ 5 file mô tả đầy đủ dự án LogiHub Intelligence trong giai đoạn midterm.*
